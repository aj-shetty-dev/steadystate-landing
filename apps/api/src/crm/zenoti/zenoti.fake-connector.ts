...
import { zenotiAppointmentFixtures, zenotiGuestFixtures } from './zenoti.fixtures';
...
export class ZenotiFakeConnector implements CrmConnector {
  ...
  async listMembers(page?: Partial<CrmPageRequest>): Promise<CrmPageResult<CrmMember>> {
    const mapped = zenotiGuestFixtures.map(zenotiGuestToCrmMember);
  ...
  async listVisits(page?: Partial<CrmPageRequest>): Promise<CrmPageResult<CrmVisit>> {
    const mapped = zenotiAppointmentFixtures.map(zenotiAppointmentToCrmVisit);
    const filtered = page?.since
