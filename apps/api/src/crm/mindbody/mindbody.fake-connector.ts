...
import { mindbodyClientFixtures, mindbodyVisitFixtures } from './mindbody.fixtures';
...
export class MindbodyFakeConnector implements CrmConnector {
  ...
  async listMembers(page?: Partial<CrmPageRequest>): Promise<CrmPageResult<CrmMember>> {
    const mapped = mindbodyClientFixtures.map(mindbodyClientToMember);
  ...
  async listVisits(page?: Partial<CrmPageRequest>): Promise<CrmPageResult<CrmVisit>> {
    const mapped = mindbodyVisitFixtures.map(mindbodyVisitToCrmVisit);
    const filtered = page?.since
