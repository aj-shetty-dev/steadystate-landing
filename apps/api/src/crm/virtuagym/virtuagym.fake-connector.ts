...
import { virtuagymMemberFixtures, virtuagymVisitFixtures } from './virtuagym.fixtures';
...
export class VirtuagymFakeConnector implements CrmConnector {
  ...
  async listMembers(page?: Partial<CrmPageRequest>): Promise<CrmPageResult<CrmMember>> {
    const mapped = virtuagymMemberFixtures.map(virtuagymMemberToCrmMember);
  ...
  async listVisits(page?: Partial<CrmPageRequest>): Promise<CrmPageResult<CrmVisit>> {
    const mapped = virtuagymVisitFixtures.map(virtuagymVisitToCrmVisit);
